...
local vipCoinEvent = ReplicatedStorage:WaitForChild("GiveVipCoinsEvent") -- [NUEVO]
...
local function updateGridVisuals()
	for _, data in ipairs(gridButtons) do
		...
		if isSelected then
			stroke.Color = Color3.new(1, 1, 1); stroke.Thickness = 3; stroke.Transparency = 0; btn.ZIndex = 30 
		else
			stroke.Color = Color3.new(0, 0, 0); stroke.Thickness = 2; stroke.Transparency = 0.5; btn.ZIndex = 27
		end
	end
end
...
local confirmColor = createSelectorButton(rgbFrame, getTxt("BTN_CONFIRM"), Color3.fromRGB(0, 200, 100), Color3.fromRGB(0, 150, 50), UDim2.new(0.5, 0, 0.96, 0), UDim2.new(0, 160, 0, 50))
confirmColor.MouseButton1Click:Connect(function()
	if currentItemToColor and colorEvent then SoundManager.Play("AbilityReady"); colorEvent:FireServer(currentItemToColor, selectedColor.R, selectedColor.G, selectedColor.B) end
...
closeColor.TextSize = 20; closeColor.ZIndex = 27; Instance.new("UICorner", closeColor)
...
local function openColorSelector(itemKey)
	if itemKey == "DoubleJumpColor" then currentItemToColor = "DoubleJump"; rgbTitle.Text = getTxt("ITEM_JUMP_COLOR")
	elseif itemKey == "DashColor" then currentItemToColor = "Dash"; rgbTitle.Text = getTxt("ITEM_DASH_COLOR")
	elseif itemKey == "BonkColor" then currentItemToColor = "Bonk"; rgbTitle.Text = getTxt("ITEM_BONK_COLOR") end
	local savedColor = player:GetAttribute(itemKey); if typeof(savedColor) ~= "Color3" then savedColor = Color3.fromRGB(255, 255, 255) end
	selectedColor = savedColor; preview.BackgroundColor3 = selectedColor; rgbFrame.Visible = true; screenGui.DisplayOrder = 25; updateGridVisuals()
end
	...
	if isOwned then
		if config.Key == "DoubleJump" or config.Key == "DashUnlock" or config.Key == "BonkUnlock" then
			...
			if config.Key == "BonkUnlock" then
				...
				shinyBtn.Size = UDim2.new(0, 200, 0, 45); shinyBtn.Position = UDim2.new(1, -145, 0.5, 0); shinyBtn.AnchorPoint = Vector2.new(1, 0.5)
			...
			acquired.FontFace = Font.fromEnum(Enum.Font.GothamBlack); acquired.TextSize = 20; acquired.ZIndex = 12; addTextStroke(acquired)
		end
	else
...
-- [MODIFICADO] BYPASS VIP PARA MONEDAS
local function renderCoinRow(pkg)
	...
	-- CHEQUEO VIP
	local isVip = VipList.IsVip(player.UserId)
	local btnText = isVip and "GRATIS (VIP)" or pkg.PriceText
	
	local buyBtn = createStyledButton(card, btnText, Color3.fromRGB(0, 200, 100), Color3.fromRGB(0, 150, 50)); 
	buyBtn.Position = UDim2.new(1, -15, 0.5, 0); buyBtn.AnchorPoint = Vector2.new(1, 0.5)
	
	-- Ajuste de texto para que entre "GRATIS (VIP)"
	if isVip then
		local txt = buyBtn:FindFirstChild("TextLabel")
		if txt then txt.TextSize = 16 end
	end
	
	buyBtn.MouseButton1Click:Connect(function() 
		SoundManager.Play("ShopButton")
		if isVip then
			-- Bypass: Dar monedas directamente a través del servidor
			vipCoinEvent:FireServer(pkg.ProductId)
		else
			-- Jugador normal: Prompt de Robux
