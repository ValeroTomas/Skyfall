...
-- 1. CREACIÓN DE VALORES PÚBLICOS (Esto es lo que faltaba)
...
while true do
	...
	-- FUNCIÓN PARA CONTAR HUMANOS + BOTS
	...
	-- Bucle de espera hasta tener mínimos jugadores
	while currentCount < MinPlayers do
		currentCount = getPlayerCount()
		estadoValue.Value = "WAITING|" .. currentCount .. "|" .. MinPlayers
		task.wait(1)
	end
	...
	for i = IntermissionTime, 1, -1 do
		-- Si se van jugadores durante la cuenta, volvemos a esperar
	...
	-- Chequeo final antes de empezar
	if getPlayerCount() >= MinPlayers then
		-------------------------------------------------------------------
		-- FASE 3: JUEGO ACTIVO
		-------------------------------------------------------------------
